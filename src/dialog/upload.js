...
// Style de fonts
import '../font/fontign.css'
import '../font/fontign.def.js'
...
function loadCarte (ripart, r, attribut) {
  ...
  for (var i=0, fi; fi=r[i]; i++) {
    ...
    if (atts) {
      ...
      f.setIgnStyle ( config.ignStyle.default[f.get(attribut)] || config.ignStyle.default.default );
      // console.log(fi);
    } else {
      f.setIgnStyle ( config.ignStyle.default.default );
    }
    features.push(f);
    // Croquis
    if (fi.croquis) {
